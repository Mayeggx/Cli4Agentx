...
// Exec runs a command string through Layer 1: pure Unix semantics.
// Handles chain operators (&&, ;, ||) and pipes (|). Returns raw output
// and the exit code of the last executed segment. No metadata is appended
// here — that happens in Layer 2 (see loop.go presentForLLM).
func (r *Registry) Exec(command, stdin string) (string, int) {
	...
	for i, seg := range segments {
		if i > 0 {
			...
			if prevOp == OpAnd && lastExitCode != 0 {
				continue
			}
			// || semantics: skip if previous succeeded
			if prevOp == OpOr && lastExitCode == 0 {
				continue
			}
		}
		...
		lastOutput, lastExitCode = r.execSingle(seg.Raw, segStdin)
...
// execSingle executes one command segment. Layer 1 — returns raw output and
// exit code with no metadata appended. Output must remain unmodified so that
// pipe chains receive clean stdin.
func (r *Registry) execSingle(command, stdin string) (string, int) {
	...
	if !ok {
		...
		return fmt.Sprintf("[error] unknown command: %s\nAvailable: %s", name, strings.Join(available, ", ")), 127
	}

	out, err := handler(args, stdin)
	if err != nil {
		return fmt.Sprintf("[error] %s: %v", name, err), 1
	}
	return out, 0
}

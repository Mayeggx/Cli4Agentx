...
- **run 是你唯一的工具** — memory、topic、skill 等都是 run 的子命令，不是独立工具。正确用法：run(command="memory search xxx")
...
文件存储在 data/topics/{topic-id}/ 目录下：
- 相对路径（如 notes.md）— 指向当前话题目录
- 绝对路径（如 /{topic-id}/file.png）— 指向指定话题目录

## 输出格式

- **数学公式**用 KaTeX 语法：行内 $E=mc^2$，独立行 $$\int_0^1 f(x)dx$$
- **图片**用本地数据协议：![描述](local-data://data/topics/{topic-id}/filename.png)
- **代码块**标注语言：` + "```python" + `
`

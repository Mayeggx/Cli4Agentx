...
func callAnthropic(provider *ProviderConfig, model string, messages []Message, tools []ToolDef, onToken func(string), onThinking func(string), logger *LLMLogger) (*LLMResponse, error) {
	system, anthMsgs := convertMessagesForAnthropic(messages)
	anthTools := convertToolsForAnthropic(tools)

	reqPayload := anthRequest{
		Model:     model,
		System:    system,
		MaxTokens: 16384,
		Stream:    true,
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		errMsg := fmt.Sprintf("anthropic error %d: %s", resp.StatusCode, string(b))
		if logger != nil {
	...
	result, err := parseAnthropicStream(resp.Body, onToken, onThinking)
	if err != nil {
		if logger != nil {
			logger.Log(provider.BaseURL, model, body, nil, err.Error(), time.Since(startTime).Milliseconds())
		}
